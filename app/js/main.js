...
var _band_model = require('./band_model');

var _band_model2 = _interopRequireDefault(_band_model);
...
var _band_collection = require('./band_collection');

var _band_collection2 = _interopRequireDefault(_band_collection);

exports.BandModel = _band_model2['default'];
exports.BandCollection = _band_collection2['default'];

},{"./band_collection":4,"./band_model":5}],7:[function(require,module,exports){
'use strict';
...
exports['default'] = _backbone2['default'].Router.extend({
  ...
  initialize: function initialize(appElement) {
    ...
    this.$el = appElement;
    this.collection = new _resources.BandCollection();

    this.$el.on('click', '.button-home', function (event) {
      var $div = (0, _jquery2['default'])(event.currentTarget);
      _this.navigate('home', { trigger: true });
    });

    this.$el.on('click', '.button-newband', function (event) {
      var $div = (0, _jquery2['default'])(event.currentTarget);
      _this.navigate('addBandProfile', { trigger: true });
    });

    this.$el.on('click', '.button-about', function (event) {
      var $div = (0, _jquery2['default'])(event.currentTarget);
      _this.navigate('about', { trigger: true });
    });

    this.$el.on('click', '.button-contact', function (event) {
      var $div = (0, _jquery2['default'])(event.currentTarget);
      _this.navigate('contact', { trigger: true });
    });
    ...
    this.$el.on('click', '.submit-band', function (event) {
      ...
      var newBand = new _resources.BandModel({
        Name: name,
        imageUrl: image,
        favoriteAlbum: favAlbum,
        Description: descript
  ...
  showHome: function showHome() {
    ...
    this.collection.fetch().then(function () {
      _this2.$el.html((0, _views.Home)(_this2.collection.toJSON()));
    });
  },

  showAbout: function showAbout() {
    this.showSpinner();
    this.$el.html((0, _views.About)());
  },

  showContact: function showContact() {
    this.showSpinner();
    this.$el.html((0, _views.Contact)());
  },
  ...
  showDetail: function showDetail(id) {
    ...
    if (band) {
      this.$el.html((0, _views.Detail)(band.templateData()));
    } else {
      this.showSpinner();
      band = this.collection.add({ objectId: id });
      band.fetch().then(function () {
        _this3.$el.html((0, _views.Detail)(band.templateData()));
      });
    }
  },
